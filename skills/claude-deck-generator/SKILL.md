...
description: AI-powered presentation generator — Claude Opus 4.7 with effort=high (full quality), Haiku draft mode (fast outline + cost preview, then upscale), or NVIDIA Palmyra. Remix existing decks with optional Phi-4 + DePlot vision pass. 4 themes, .pptx and .html output.
---

# Claude Deck Generator

AI-powered presentation generator. Produces polished `.pptx` or `.html` slide decks using Claude Opus 4.7 with adaptive thinking and `effort=high` — or Haiku for fast draft previews before upscaling.
...
### Provider: Anthropic vs Haiku vs NVIDIA?

| `--provider anthropic` (default) | `--provider claude-haiku` | `--provider nvidia` |
|---|---|---|
| Best narrative quality, Opus 4.7 + adaptive thinking + effort=high | Fast, cheap — Haiku 4.5, no adaptive thinking | Lower cost per run, NVIDIA infra |
| Structured output via SLIDE_SCHEMA | Same JSON output, 4 000-token ceiling | Same JSON output — identical downstream rendering |
| Use for final/production decks | Use for quick iterations or volume runs | Use for drafts or when on NVIDIA infra |
...
| `--provider` | `anthropic` | `anthropic` (Opus 4.7, default) · `claude-haiku` (Haiku 4.5, fast draft) · `nvidia` (Palmyra via NIM) |
...
1. **Content generation** — `generate_content()` calls Claude Opus 4.7 with `thinking: {type: "adaptive"}` and `output_config: {effort: "high"}`, enforcing `SLIDE_SCHEMA` — structured JSON every time. Haiku variant (`generate_content_haiku()`) is used for `--provider claude-haiku` or draft previews.
...
- `--provider claude-haiku` uses Haiku 4.5 without adaptive thinking; useful for fast drafts or high-volume runs
